-- Staff-scope Opening PAR completion counts for multi-staff Barback links.
-- This redefines public.barback_confirm_opening_par from the latest conflict-target migration.
...
DECLARE
  ...
  v_staff_allowed_bar_ids uuid[] := ARRAY[]::uuid[];
...
BEGIN
  ...
  -- BARBACK_OPENING_PAR_RPC_STAFF_SCOPED_DESTINATIONS_START
  -- Multi-staff links store the union of all destinations on the session.
  -- Completion must be counted only against the destinations assigned to the
  -- staff member who entered the PIN.
  SELECT COALESCE(nsa.allowed_bar_ids, ARRAY[]::uuid[])
    INTO v_staff_allowed_bar_ids
  FROM public.night_staff_assignments AS nsa
  WHERE nsa.night_id = p_night_id
    AND nsa.staff_id = p_staff_id
    AND COALESCE(nsa.revoked, false) IS FALSE
  LIMIT 1;

  IF v_staff_allowed_bar_ids IS NULL
     OR COALESCE(array_length(v_staff_allowed_bar_ids, 1), 0) = 0 THEN
    RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'staff has no assigned destinations for this night';
  END IF;

  SELECT COALESCE(array_agg(scope.destination_id ORDER BY scope.destination_id), ARRAY[]::uuid[])
    INTO v_destination_ids
  FROM (
    SELECT session_scope.destination_id
    FROM (
    ...
    WHERE session_scope.destination_id = ANY(v_staff_allowed_bar_ids)
  ) AS scope;

  IF COALESCE(array_length(v_destination_ids, 1), 0) = 0 THEN
    RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'staff has no assigned destinations in this Opening PAR session';
  END IF;
  -- BARBACK_OPENING_PAR_RPC_STAFF_SCOPED_DESTINATIONS_END

  IF NOT (p_destination_id = ANY(v_destination_ids)) THEN
    RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'destination is outside this staff assignment';
  END IF;
  IF NOT EXISTS (
    SELECT 1
